...
The live job runs `bl login` with its service-account key before testing and `bl logout` in an always-run cleanup step. CLI `0.1.110` reads its saved login for `bl connect`; environment variables alone authenticate SDK calls but do not initialize that terminal login. The saved workspace credential is removed after the test. A local run with an existing CLI login does not prove this fresh-runner setup.
